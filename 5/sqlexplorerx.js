...
function initSQLExplorer() {
    ...
    // Add Telegram backup button event listener
    const sqlBackupToTelegram = document.getElementById('sqlBackupToTelegram');
    if (sqlBackupToTelegram) {
        sqlBackupToTelegram.addEventListener('click', function() {
            backupAllDatabasesToTelegram();
        });
    }
...
/**
 * Backup all databases to Telegram with encryption
 */
function backupAllDatabasesToTelegram() {
    sqlDebugLog('Backing up all databases to Telegram');
    
    // Show confirmation dialog first
    Swal.fire({
        title: 'Backup All Databases to Telegram',
        text: 'This will export all databases, encrypt them with a random password, and send them to your configured Telegram bot. The password will be included in the message. Continue?',
        icon: 'question',
        ...
        if (result.isConfirmed) {
            showLoadingState('Preparing database backup for Telegram...');
            
            // Get export options - use compressed with structure and data
            const data = {
                csrf: getCsrfToken(),
                action: 'sql_explorer',
                sql_action: 'backup_all_databases_telegram',
                params: {
            ...
            // This may take some time, so use a fetch with longer timeout
            sqlSendRequest(data)
                .then(result => {
                    hideLoadingState();
                    sqlDebugLog('Backup to Telegram result', result);
                    
                    if (result.success) {
                        // Display success message with password info
                        Swal.fire({
                            ...
                            html: `
                                <div class="text-left">
                                    ...
                                    <ul class="list-disc pl-5">
                                        <li>Size: ${formatFileSize(result.filesize || 0)}</li>
                                        <li>Encrypted with password: <code class="bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">${result.password || 'N/A'}</code></li>
                                    </ul>
                        ...
                        triggerAlert('warning', 'Failed to backup databases to Telegram: ' + (result.error || 'Unknown error'));
                    }
                })
                .catch(error => {
                    hideLoadingState();
                    sqlDebugLog('Backup to Telegram error', error);
                    triggerAlert('warning', 'Error during backup: ' + error);
                });
        }
    });
}
